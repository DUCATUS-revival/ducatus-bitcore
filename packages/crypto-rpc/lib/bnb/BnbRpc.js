...
class BnbRPC {
  ...
  async sendToAddress({
    ...
    const amountStr = Number(amount).toLocaleString("fullwide", {
      useGrouping: false
    });
    const sendParams = {
      from: account,
  ...
  async unlockAndSendToAddress({
    ...
    const tx = await this.sendToAddress({
      address,
      amount,
      fromAccount,
      passphrase,
  ...
  async unlockAndSendToAddressMany({
    payToArray,
    passphrase,
    fromAccount,
    gasPrice,
  ...
  async estimateGasPrice() {
    ...
    const shortAverage = Math.ceil(
      medianGasPrices
        .slice(0, medianGasPrices.length / 2)
        .reduce((acc, cur) => acc + cur, 0) /
        (medianGasPrices.length / 2)
    );
    const longAverage = Math.ceil(
      medianGasPrices.reduce((acc, cur) => acc + cur, 0) /
        medianGasPrices.length
    );
