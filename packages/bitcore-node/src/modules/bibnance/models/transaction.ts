...
import { BnbTransactionJSON, IBnbTransaction } from '../types';
...
@LoggifyClass
export class BnbTransactionModel extends BaseTransaction<IBnbTransaction> {
  ...
  onConnect() {
    ...
    this.collection.createIndex(
      ...
      {
        background: true,
        partialFilterExpression: { chain: 'BNB', 'abiType.type': 'ERC20', 'abiType.name': 'transfer' }
      }
    );
  }

  async batchImport(params: {
    txs: Array<IBnbTransaction>;
    height: number;
    mempoolTime?: Date;
    blockTime?: Date;
    blockHash?: string;
    ...
    if (params.height < SpentHeightIndicators.minimum) {
      for (let op of txOps) {
        ...
        const tx = { ...op.updateOne.update.$set, ...filter } as IBnbTransaction;
  ...
  async addTransactions(params: {
    ...
    if (parentChain && forkHeight && height < forkHeight) {
      const parentTxs = await BnbTransactionStorage.collection
      ...
      return Promise.all(
        params.txs.map(async (tx: IBnbTransaction) => {
  ...
  getTransactions(params: { query: any; options: StreamingFindOptions<IBnbTransaction> }) {
  ...
  _apiTransform(
    tx: IBnbTransaction | Partial<MongoBound<IBnbTransaction>>,
    options?: TransformOptions
  ): BnbTransactionJSON | string {
    const dataStr = `0x${tx.data!.toString('hex')}`;
    const decodedData = this.abiDecode(dataStr);

    const transaction: BnbTransactionJSON = {
      txid: tx.txid || '',
      network: tx.network || '',
      chain: tx.chain || '',
...
export let BnbTransactionStorage = new BnbTransactionModel();
