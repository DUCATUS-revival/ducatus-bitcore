...
import { IBnbBlock, IBnbTransaction } from '../types';
import { BnbTransactionStorage } from './transaction';

@LoggifyClass
export class BnbBlockModel extends BaseBlock<IBnbBlock> {
  ...
  async processBlock(params: {
    ...
    await BnbTransactionStorage.batchImport({
      txs: transactions,
  ...
  async getBlockOp(params: { block: IBnbBlock; chain: string; network: string }) {
    const { block, chain, network } = params;
  ...
  async handleReorg(params: { block: IBnbBlock; chain: string; network: string }): Promise<boolean> {
    const { block, chain, network } = params;
    const prevHash = block.previousBlockHash;
    ...
    const reorgOps = [
      ...
      BnbTransactionStorage.collection.deleteMany({ chain, network, blockHeight: { $gte: localTip.height } })
    ];
  ...
  _apiTransform(block: Partial<MongoBound<IBnbBlock>>, options?: TransformOptions): any {
    const transform = {
      _id: block._id,
      chain: block.chain,
...
export let BnbBlockStorage = new BnbBlockModel();
