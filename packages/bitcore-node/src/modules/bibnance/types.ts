...
export type Networks = 'smart chain';

export interface BinanceBlock {
  header: BinanceHeader;
  transactions: Transaction[];
  uncleHeaders: BinanceHeader[];
  raw: Buffer[];
  txTrie: any;
}

export interface BinanceHeader {
  parentHash: Buffer;
  uncleHash: Buffer;
  coinbase: Buffer;
  stateRoot: Buffer;
...
export type IBnbTransaction = ITransaction & {
  data: Buffer;
  gasLimit: number;
  gasPrice: number;
  nonce: number;
...
export interface BnbTransactionJSON {
  txid: string;
  chain: string;
  network: string;
  blockHeight: number;
  ...
  to: string;
  from: string;
  abiType?: IBnbTransaction['abiType'];
