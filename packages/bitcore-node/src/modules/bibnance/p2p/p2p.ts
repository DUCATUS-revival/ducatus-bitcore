...
import { BnbBlockModel, BnbBlockStorage } from '../models/block';
import { BnbTransactionModel, BnbTransactionStorage } from '../models/transaction';
import { IBnbBlock, IBnbTransaction, ParityBlock, ParityTransaction } from '../types';
import { ParityRPC } from './parityRpc';

export class BnbP2pWorker extends BaseP2PWorker<IBnbBlock> {
  protected chainConfig: any;
  protected syncing: boolean;
  protected initialSyncComplete: boolean;
  protected blockModel: BnbBlockModel;
  protected txModel: BnbTransactionModel;
  protected txSubscription: any;
  protected blockSubscription: any;
  protected rpc?: ParityRPC;
  protected provider: BNBStateProvider;
  protected web3?: Web3;
  ...
  constructor({ chain, network, chainConfig, blockModel = BnbBlockStorage, txModel = BnbTransactionStorage }) {
    super({ chain, network, chainConfig, blockModel });
    this.chain = chain || 'BNB';
    ...
    this.provider = new BNBStateProvider();
  ...
  async processBlock(block: IBnbBlock, transactions: IBnbTransaction[]): Promise<any> {
  ...
  async convertBlock(block: ParityBlock) {
    ...
    const convertedBlock: IBnbBlock = {
      chain: this.chain,
      network: this.network,
      height,
      hash,
  ...
  convertTx(tx: Partial<ParityTransaction>, block?: IBnbBlock): IBnbTransaction {
    if (!block) {
      const txid = tx.hash || '';
      const to = tx.to || '';
      const from = tx.from || '';
      ...
      const convertedTx: IBnbTransaction = {
        chain: this.chain,
        network: this.network,
