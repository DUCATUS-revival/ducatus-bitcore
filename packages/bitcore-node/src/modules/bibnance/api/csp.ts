...
import { BnbBlockStorage } from '../models/block';
import { BnbTransactionStorage } from '../models/transaction';
import { BnbTransactionJSON, IBnbBlock } from '../types';
import { BnbListTransactionsStream } from './transform';
...
export class BNBStateProvider extends InternalStateProvider implements IChainStateService {
  config: any;
  static rpcs = {} as { [network: string]: { rpc: CryptoRpc; web3: Web3 } };

  constructor(public chain: string = 'BNB') {
    super(chain);
  ...
  async getWeb3(network: string): Promise<{ rpc: CryptoRpc; web3: Web3 }> {
    try {
      if (BNBStateProvider.rpcs[network]) {
        await BNBStateProvider.rpcs[network].web3.eth.getBlockNumber();
      }
    } catch (e) {
      delete BNBStateProvider.rpcs[network];
    }
    if (!BNBStateProvider.rpcs[network]) {
      ...
      BNBStateProvider.rpcs[network] = { rpc, web3: rpc.web3 };
    }
    return BNBStateProvider.rpcs[network];
  }
  ...
  async getERC20TokenInfo(network: string, tokenAddress: string) {
    const token = await BNB.erc20For(network, tokenAddress);
  ...
  async getFee(params) {
    ...
    const txs = await BnbTransactionStorage.collection
  ...
  async getLocalTip({ chain, network }) {
    return BnbBlockStorage.getLocalTip({ chain, network });
  }
  ...
  async getTransaction(params: StreamTransactionParams) {
    try {
      ...
      const found = await BnbTransactionStorage.collection.findOne(query);
      if (found) {
        let confirmations = 0;
        if (found.blockHeight && found.blockHeight >= 0) {
          confirmations = tipHeight - found.blockHeight + 1;
        }
        const convertedTx = BnbTransactionStorage._apiTransform(found, { object: true }) as BnbTransactionJSON;
  ...
  async streamAddressTransactions(params: StreamAddressUtxosParams) {
    ...
    if (!args.tokenAddress) {
      ...
      Storage.apiStreamingFind(BnbTransactionStorage, query, { limit, since, paging: '_id' }, req, res);
    } else {
      try {
  ...
  async streamTransactions(params: StreamTransactionsParams) {
    ...
    const tip = await this.getLocalTip(params);
    const tipHeight = tip ? tip.height : 0;
    return Storage.apiStreamingFind(BnbTransactionStorage, query, args, req, res, t => {
      let confirmations = 0;
      if (t.blockHeight !== undefined && t.blockHeight >= 0) {
        confirmations = tipHeight - t.blockHeight + 1;
      }
      const convertedTx = BnbTransactionStorage._apiTransform(t, { object: true }) as Partial<ITransaction>;
  ...
  async streamWalletTransactions(params: StreamWalletTransactionsParams) {
    ...
    if (!args.tokenAddress) {
      transactionStream = BnbTransactionStorage.collection
        .find(query)
    ...
    const listTransactionsStream = new BnbListTransactionsStream(wallet);
  ...
  async getAccountNonce(network: string, address: string) {
    ...
    return count;
    /*
     *return BnbTransactionStorage.collection.countDocuments({
     *  chain: 'BNB',
     *  network,
     *  from: address,
     *  blockHeight: { $gt: -1 }
  ...
  async getBlocks(params: GetBlockParams) {
    ...
    let cursor = BnbBlockStorage.collection.find(query, options).addCursorFlag('noCursorTimeout', true);
    if (options.sort) {
    ...
    const tip = await this.getLocalTip(params);
    const tipHeight = tip ? tip.height : 0;
    const blockTransform = (b: IBnbBlock) => {
      let confirmations = 0;
      if (b.height && b.height >= 0) {
        confirmations = tipHeight - b.height + 1;
      }
      const convertedBlock = BnbBlockStorage._apiTransform(b, { object: true }) as IBnbBlock;
  ...
  async updateWallet(params: UpdateWalletParams) {
    ...
    for (let addressBatch of addressBatches) {
      ...
      await BnbTransactionStorage.collection.updateMany(
